...
export default function ManagerReviewsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-2xl font-extrabold text-slate-900 mb-6">Manage Reviews</h1>

      <div className="grid gap-4">
        {reviews.map(r => (
          <div key={r.review_id} className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col md:flex-row gap-6 shadow-sm">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <div className="font-bold text-lg text-slate-900">{r.reviewer_name}</div>
              ...
              {replyingTo === r.review_id && (
                <div className="mt-4 flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setReplyingTo(null)} className="px-4 py-2 bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-sm font-semibold hover:bg-slate-200">Cancel</button>
                  </div>
                </div>
              )}
            </div>
